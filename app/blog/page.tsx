...
const typeConfig: Record<ContentType, { label: string; color: string; icon: typeof BookOpen }> = {
  ...
  comparison: { label: "Comparison", color: "bg-[#faf6f0] text-[#8b2500]", icon: GitCompare },
    ...
    <div className="min-h-screen bg-[#f5f0eb]">
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <Link href="/blog" className="text-sm font-medium text-[#8b2500] py-2 px-1">
            Resources
          </Link>
          <Link
            href="/step-1"
            ...
            {content.map((piece) => {
              ...
              return (
                <Link
                  ...
                  className="group bg-white rounded-none border border-[#d4c5b0] p-5 md:p-6 hover:shadow-xl transition-all duration-300"
                >
                  ...
                  <h2 className="text-xl font-bold text-gray-900 mb-3 group-hover:text-[#8b2500] transition-colors">
                    {piece.title}
                  </h2>
                  <p className="text-gray-600 leading-relaxed mb-4 line-clamp-3">
                    {piece.description}
                  </p>
                  <span className="inline-flex items-center text-sm font-medium text-[#8b2500] group-hover:text-[#8b2500]">
                    Read more
