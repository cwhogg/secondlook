...
export default function OldHomePage() {
  return (
    <div className="min-h-screen bg-[#f5f0eb]">
      {/* Archived-version ribbon. Sticky so it's visible while scrolling. */}
      <div className="sticky top-0 z-50 bg-amber-100 border-b border-amber-300">
        <div className="max-w-[1140px] mx-auto px-4 sm:px-8 py-2 flex flex-wrap items-center justify-between gap-2">
          <div className="text-xs sm:text-sm text-amber-900">
            <span className="font-semibold uppercase tracking-wider">Archived</span>
            <span className="mx-2 text-amber-500">·</span>
            <span>You&rsquo;re viewing the previous landing page for comparison.</span>
          </div>
          <Link
            href="/"
            className="text-xs sm:text-sm font-semibold text-amber-900 underline hover:text-amber-950"
          >
            View current landing page &rarr;
          </Link>
        </div>
      </div>
      ...
      {/* Intentionally no JSON-LD script here — the current homepage owns
          the site's structured data. This archived route is noindex/nofollow
          via /app/old/layout.tsx so it never competes for SEO. */}
