...
- MERGE GUIDANCE — label preservation (MANDATORY): every 'merge' suggestion MUST specify a "mergeInto" field naming EXACTLY which of the two labels survives. The finalizer will use this label verbatim. Choose the label that carries MORE clinical information — parenthetical modifiers (e.g., "(post-COVID)", "(late-onset)", "(EPM2A/NHLRC1)"), criteria references ("fulfilling ICC criteria", "meeting Ghent criteria"), etiologic qualifiers, or specific subtype markers. Do NOT strip these modifiers by picking the shorter label. Concrete example that caused a real regression: merging "Myalgic Encephalomyelitis / Chronic Fatigue Syndrome (post-COVID) fulfilling ICC criteria" with "Chronic Fatigue Syndrome" — mergeInto MUST be the longer label with the criteria reference and etiologic modifier, NOT the shorter generic one. Downstream reads mergeInto as the single canonical name; if you omit it, the finalizer defaults to the less-informative label and clinical nuance is lost.
  ...
  "suggestions": [
    {
      ...
      "mergeInto": "<REQUIRED for 'merge' actions: EXACT surviving label the finalizer must use. Pick the label with more clinical modifiers (parentheticals, criteria refs, etiologic qualifiers, subtype markers). Omit for non-merge actions.>",
  ...
  async execute(opts: {
    ...
    const suggestions: CritiqueSuggestion[] = rawSuggestions
      ...
      .map((s: any) => ({
        ...
        mergeInto: typeof s.mergeInto === 'string' && s.mergeInto.trim() ? s.mergeInto.trim() : undefined,
