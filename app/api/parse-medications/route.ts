...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`[parse-medications] request — count=${medications?.length ?? 0}`)
...
`
    ...
    console.log(`[parse-medications] openai status=${response.status} elapsed=${responseTime}ms`)
    ...
    try {
      ...
      console.log(`[parse-medications] parsed count=${parsedMedications.medications?.length ?? 0}`)
      return NextResponse.json(parsedMedications)
    } catch (parseError: any) {
      console.error(`[parse-medications] parse failed: ${parseError?.message ?? 'unknown'}`)
