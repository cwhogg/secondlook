    ...
    <div className="min-h-screen bg-[#f5f0eb]">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 lg:py-12">
        ...
        <div className="text-center mb-8 sm:mb-12">
          {/* Animated Brain Icon */}
          <div className="inline-flex items-center justify-center w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24 bg-[#8b2500] rounded-full mb-6 sm:mb-8 animate-pulse">
        ...
        <div className="mb-8 sm:mb-12">
          <div className="flex items-center justify-between mb-2 sm:mb-3 px-2">
            ...
            <span className="text-xs sm:text-sm font-bold text-[#8b2500]">{progress}%</span>
          </div>
          <div className="w-full bg-gray-200 rounded-full h-2 sm:h-3 overflow-hidden">
            <div
              className="h-full bg-[#8b2500] rounded-full transition-all duration-500 ease-out"
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 md:gap-6 mb-8 sm:mb-12">
          {analysisSteps.map((step) => (
            <div
              key={step.id}
              className={`relative flex flex-col items-center p-3 sm:p-4 rounded-none border-2 transition-all duration-300 ${
                step.completed
            ...
            >
              ...
              <div
                className={`flex items-center justify-center w-10 h-10 sm:w-12 sm:h-12 rounded-full mb-2 sm:mb-3 ${
                  step.completed ? "bg-green-500" : progress > (step.id - 1) * 25 ? "bg-[#8b2500]" : "bg-gray-200"
                }`}
              >
                {step.completed ? (
              ...
              <span
                className={`text-xs sm:text-sm text-center font-medium ${
                  step.completed ? "text-green-700" : progress > (step.id - 1) * 25 ? "text-[#8b2500]" : "text-gray-500"
                }`}
              >
                {step.label}
              </span>
        ...
        {/* Educational Content */}
        <div className="bg-white rounded-none p-6 sm:p-8 md:p-10 border border-[#d4c5b0]">
          <div className="flex items-start gap-4 sm:gap-6">
            <div className="flex-shrink-0">
              <div className="w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 bg-[#faf6f0] rounded-full flex items-center justify-center">
                <Brain className="w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 text-[#8b2500]" />
              </div>
            </div>
            ...
            {educationalContent.map((_, index) => (
              <div
                ...
                className={`h-1.5 sm:h-2 rounded-full transition-all duration-300 ${
                  index === educationalTip ? "w-8 sm:w-10 bg-[#8b2500]" : "w-1.5 sm:w-2 bg-gray-300"
                }`}
              />
            ))}
          </div>
        ...
        {analysisData && (
          <div className="mt-6 sm:mt-8 p-4 sm:p-6 bg-gray-50 rounded-none text-xs sm:text-sm">
